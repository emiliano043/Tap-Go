document.addEventListener("DOMContentLoaded", async function () {
    const API_URL = "http://localhost:7034/api/menu";
    ...
    try {
        ...
        const bebidas = productos.filter(p => p.tipo.toLowerCase() === "bebida");

        const categorias = [...new Set(bebidas.map(a => a.categoria))];
        ...
        categorias.forEach(categoria => {
            ...
            bebidas.filter(a => a.categoria === categoria).forEach(item => {
        ...
        console.error(error);
        document.getElementById('contenedor-bebidas').innerHTML = "<p>Error al cargar productos.</p>";
    }
});
