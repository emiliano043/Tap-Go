document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        const postres = productos.filter(p => p.tipo.toLowerCase() === "postre");

        const categorias = [...new Set(postres.map(a => a.categoria))];
        ...
        categorias.forEach(categoria => {
            ...
            postres.filter(a => a.categoria === categoria).forEach(item => {
        ...
        console.error(error);
        document.getElementById('contenedor-postres').innerHTML = "<p>Error al cargar productos.</p>";
    }
});
